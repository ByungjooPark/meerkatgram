/**
 * @file app/middlewares/multer/uploders/profileUploder.js
 * @description profileUploder
 * 251019 v1.0 meerkat
 */
...
export function profileUploder(request, response, next){
  const upload = multer({
    storage: multer.diskStorage({
      // 파일 저장 경로 설정
      destination(request, file, callback) {
        const fullPath = pathUtil.getStorageProfileDirPath(); // 파일 저장 디렉토리 생성
